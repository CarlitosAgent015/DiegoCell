...
export async function goOutVehicle(req, res) {
    try {
        ...
        // Restablecer todos los campos relacionados con el vehículo estacionado
        celda.estado = 'available';  // La celda vuelve a estar disponible
        celda.plateVehicle = null;   // Limpiar la placa del vehículo
        celda.entry_date = null;     // Limpiar la fecha de entrada
        celda.departure_date = null; // Limpiar la fecha de salida
        celda.pin = null;            // Limpiar cualquier PIN asociado


        await celda.save();

        // Responder con la información del valor a pagar y las fechas
        res.json({
            message: 'The vehicle has left, and the cell is reset',
            valor_a_pagar,
            fecha_entrada: null,  // Restablecido a null
            fecha_salida: null    // Restablecido a null
        });
    } catch (error) {
